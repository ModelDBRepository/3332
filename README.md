...
- the activation steady-state,
- the time constants,
- a simple simulation using a somatic current injection of -1.3nA, and
- a family of curves generated modeling the same protocol used in the experiments for Fig.6A of the paper.

For Fig.6A the peak conductance has been adjusted to give approximately the same currents shown in the paper.

The time constants reported in the paper are shown
as markers of different colors.
Time constants in the range not covered by the paper are an arbitrary extension.
...
To run the simulation hoc file use the command:
`nrngui kinetics.hoc`
